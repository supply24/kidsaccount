...
const newKontox1 = accounter.flatMap(item=>item).filter(item =>item.kontox_value==="112")
const newKontox3 = accounter1.flatMap(item=>item).filter(item =>item.kontox_value==="112")
...
<tbody>
  ...
  {newKontox1.map((comp, index)=>(
    <tr key={index}>
    <td>{comp.kontox1.value}</td>
        <td>{comp.kredit.value}</td>
           </tr>
  ))}
</tbody>  

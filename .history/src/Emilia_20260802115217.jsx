...
const newTime1 = accounter.flatMap(item=>item).filter(item =>item.kontox_value==="112" ||item.kontox_value==="199") 
const newArr1 = newTime1.reduce((acc, item) => acc + parseFloat(item.debet_value), 0);
const newKontox1 = accounter.flatMap(item=>item).filter(item =>item.kontox1_value==="112" ||item.kontox1_value==="199")

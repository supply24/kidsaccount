...
const newTime3 = accounter1.flatMap(item=>item).filter(item =>item.kontox_value==="112")
const newArr1 = newTime3.reduce((acc, item) => acc + parseFloat(item.sumx), 0);
console.log("newArr1", newArr1)

const newKontox1 = accounter.flatMap(item=>item).filter(item =>item.kontox1_value==="112")
const newKontox3 = accounter1.flatMap(item=>item).filter(item =>item.kontox1_value==="112")
const newArr = newKontox3.reduce((acc, item) => acc + parseFloat(item.newsum), 0);
